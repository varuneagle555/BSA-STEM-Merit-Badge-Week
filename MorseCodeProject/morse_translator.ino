//TODO: Assign the pins of the Morse Code Machine
const int buttonPin = ;    
const int ledPin = ;      
const int buzzer = ;
...
void loop() {
  ...
  else if(!buttonState && lastButtonState)          //this part of the code happens when the button is released and it send either * or - into the buffer
  {
     ...
     if (signal_length>50 && signal_length<2*pause_value) //TODO If Dot Detected, add a dot to the string
     {
       morse =  ;
     } 

     //TODO If Dash Detected (signal_length>2*pause_value), add a dash to the string
  ...
  //TODO set Last Button state to current button state
  delay(1);
}
...
{                                 //compare morse string to known morse values and print out the letter or a number 
                                  ...
                                  //TODO Fill in Morse Code Values
  if (prevodilac=="")
    Serial.print("A");
  else if (prevodilac=="")  
    Serial.print("B");
